...
public class MenuFichaCochePolicia extends JFrame implements KeyListener, ActionListener {
	...
	public MenuFichaCochePolicia(){
        ...
        setResizable(false); //Que non se poda cpoliar o tamaanho da ventana
        ...
        fic = new Ficha();
        pol = new Policia();
        sfechaEntrada = pol.getfechaAlta(pol.getUltimoRegitro());
        scliente = pol.getNombrecliente(pol.getUltimoRegitro());
        svehiculo = pol.getNombrevehiculo(pol.getUltimoRegitro());
...
idvehi = pol.getUltimoRegitro();
		
		if (pol.getCombustible(idvehi).equals("diésel")){
			inputdescripcion.setText("Revisión del filtro de partículas");
			if (pol.getNombretipovehiculo(idvehi).equals("polulancia") || pol.getNombretipovehiculo(idvehi).equals("Bomberos") || pol.getNombretipovehiculo(idvehi).equals("Policía")) {
				inputdescripcion.setText("Revisión del filtro de partículas, Revisión de la sirena");	
			}
		}
		else if (pol.getnRuedas(idvehi).equals("2")) {
		...
		else if (pol.getNombretipovehiculo(idvehi).equals("polulancia") || pol.getNombretipovehiculo(idvehi).equals("Bomberos") || pol.getNombretipovehiculo(idvehi).equals("Policía")) {
	...
	public void actionPerformed(ActionEvent e) {		
		...
		if (e.getActionCommand().equals("ENVIAR")) {
				...
				if (fic.editar(pol.getUltimoRegitro(), inputdescripcion.getText(), sest, inputfechaSalida.getText(), inputmotivo.getText())) {
