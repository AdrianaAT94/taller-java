...
public class MenuFichaDeportivo extends JFrame implements KeyListener, ActionListener {
	...
	public MenuFichaDeportivo(){
        ...
        setResizable(false); //Que non se poda cdepiar o tamaanho da ventana
        ...
        fic = new Ficha();
        dep = new Deportivo();
        sfechaEntrada = dep.getfechaAlta(dep.getUltimoRegitro());
        scliente = dep.getNombrecliente(dep.getUltimoRegitro());
        svehiculo = dep.getNombrevehiculo(dep.getUltimoRegitro());
...
idvehi = dep.getUltimoRegitro();
		
		if (dep.getCombustible(idvehi).equals("diésel")){
			inputdescripcion.setText("Revisión del filtro de partículas");
			if (dep.getNombretipovehiculo(idvehi).equals("depulancia") || dep.getNombretipovehiculo(idvehi).equals("Bomberos") || dep.getNombretipovehiculo(idvehi).equals("Policía")) {
				inputdescripcion.setText("Revisión del filtro de partículas, Revisión de la sirena");	
			}
		}
		else if (dep.getnRuedas(idvehi).equals("2")) {
		...
		else if (dep.getNombretipovehiculo(idvehi).equals("depulancia") || dep.getNombretipovehiculo(idvehi).equals("Bomberos") || dep.getNombretipovehiculo(idvehi).equals("Policía")) {
	...
	public void actionPerformed(ActionEvent e) {		
		...
		if (e.getActionCommand().equals("ENVIAR")) {
				...
				if (fic.editar(dep.getUltimoRegitro(), inputdescripcion.getText(), sest, inputfechaSalida.getText(), inputmotivo.getText())) {
