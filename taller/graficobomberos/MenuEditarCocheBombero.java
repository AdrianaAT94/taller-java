...
public class MenuEditarCocheBombero extends JFrame implements KeyListener, ActionListener {
	...
	public MenuEditarCocheBombero(){
        ...
        smatricula = bom.getMatricula(MenuEditarCocheBomberoLista.id);
        smarca = bom.getMarca(MenuEditarCocheBomberoLista.id);
        smodelo = bom.getModelo(MenuEditarCocheBomberoLista.id);
        scombustible = bom.getCombustible(MenuEditarCocheBomberoLista.id);
        snPuertas = bom.getnPuertas(MenuEditarCocheBomberoLista.id);
        snRuedas = bom.getnRuedas(MenuEditarCocheBomberoLista.id);
        smaxVelocidad = bom.getMaxVelocidad(MenuEditarCocheBomberoLista.id);
        scapMotor = bom.getCapMotor(MenuEditarCocheBomberoLista.id);
        scapAlmacenamiento = bom.getCapAlmacenamiento(MenuEditarCocheBomberoLista.id);
        snPasajeros = bom.getnPasajeros(MenuEditarCocheBomberoLista.id);
        sABS = bom.isABS(MenuEditarCocheBomberoLista.id);
        sGPS = bom.isGPS(MenuEditarCocheBomberoLista.id);
        sairbags = bom.isAirbags(MenuEditarCocheBomberoLista.id);
        sclimatizacion = bom.isClimatizacion(MenuEditarCocheBomberoLista.id);
        sdescapotable = bom.isDescapotable(MenuEditarCocheBomberoLista.id);
        sduenho = bom.getIdcliente(MenuEditarCocheBomberoLista.id);
        smotivoVisita = bom.getMotivoVisita(MenuEditarCocheBomberoLista.id);
      ...
      //Creanse paneles e daselle un color de fondo
        ...
        label = new JLabel("EDITAR COCHE DE BOMBERO");
	...
	public void actionPerformed(ActionEvent e) {	
		...
		if (e.getActionCommand().equals("ATRÁS")) {
			MenuCochesBomberos men = new MenuCochesBomberos();
			dispose();
			men.setVisible(true);
		...
		if (e.getActionCommand().equals("ENVIAR")) {
	        ...
	        if (bom.editar(MenuEditarCocheBomberoLista.id, inputmarca.getText(), inputmodelo.getText(), inputmatricula.getText(), combus, inputnPuertas.getText(), inputnRuedas.getText(), inputmaxVelocidad.getText(), inputcapMotor.getText(), inputcapAlmacenamiento.getText(), inputnPasajeros.getText(), a, g, airb, clim, des, idcli, inputmotivoVisita.getText())) {
		        MenuListadoCochesBomberos men = new MenuListadoCochesBomberos();
				dispose();
				men.setVisible(true);
				...
				MenuErrorRegistroCocheBombero men = new MenuErrorRegistroCocheBombero();
				men.setVisible(true);
				dispose();
