...
public class MenuFichaCocheBombero extends JFrame implements KeyListener, ActionListener {
	...
	public MenuFichaCocheBombero(){
        ...
        setResizable(false); //Que non se poda cbomiar o tamaanho da ventana
        ...
        fic = new Ficha();
        bom = new Bombero();
        sfechaEntrada = bom.getfechaAlta(bom.getUltimoRegitro());
        scliente = bom.getNombrecliente(bom.getUltimoRegitro());
        svehiculo = bom.getNombrevehiculo(bom.getUltimoRegitro());
...
idvehi = bom.getUltimoRegitro();
		
		if (bom.getCombustible(idvehi).equals("diésel")){
			inputdescripcion.setText("Revisión del filtro de partículas");
			if (bom.getNombretipovehiculo(idvehi).equals("bomulancia") || bom.getNombretipovehiculo(idvehi).equals("Bomberos") || bom.getNombretipovehiculo(idvehi).equals("Policía")) {
				inputdescripcion.setText("Revisión del filtro de partículas, Revisión de la sirena");	
			}
		}
		else if (bom.getnRuedas(idvehi).equals("2")) {
		...
		else if (bom.getNombretipovehiculo(idvehi).equals("bomulancia") || bom.getNombretipovehiculo(idvehi).equals("Bomberos") || bom.getNombretipovehiculo(idvehi).equals("Policía")) {
	...
	public void actionPerformed(ActionEvent e) {		
		...
		if (e.getActionCommand().equals("ENVIAR")) {
				...
				if (fic.editar(bom.getUltimoRegitro(), inputdescripcion.getText(), sest, inputfechaSalida.getText(), inputmotivo.getText())) {
