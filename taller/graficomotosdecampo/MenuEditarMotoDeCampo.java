...
public class MenuEditarMotoDeCampo extends JFrame implements KeyListener, ActionListener {
	...
	public MenuEditarMotoDeCampo(){
        ...
        smatricula = cam.getMatricula(MenuEditarMotosDeCampoLista.id);
        smarca = cam.getMarca(MenuEditarMotosDeCampoLista.id);
        smodelo = cam.getModelo(MenuEditarMotosDeCampoLista.id);
        scombustible = cam.getCombustible(MenuEditarMotosDeCampoLista.id);
        snPuertas = cam.getnPuertas(MenuEditarMotosDeCampoLista.id);
        snRuedas = cam.getnRuedas(MenuEditarMotosDeCampoLista.id);
        smaxVelocidad = cam.getMaxVelocidad(MenuEditarMotosDeCampoLista.id);
        scapMotor = cam.getCapMotor(MenuEditarMotosDeCampoLista.id);
        scapAlmacenamiento = cam.getCapAlmacenamiento(MenuEditarMotosDeCampoLista.id);
        snPasajeros = cam.getnPasajeros(MenuEditarMotosDeCampoLista.id);
        sABS = cam.isABS(MenuEditarMotosDeCampoLista.id);
        sGPS = cam.isGPS(MenuEditarMotosDeCampoLista.id);
        sairbags = cam.isAirbags(MenuEditarMotosDeCampoLista.id);
        sclimatizacion = cam.isClimatizacion(MenuEditarMotosDeCampoLista.id);
        sdescapotable = cam.isDescapotable(MenuEditarMotosDeCampoLista.id);
        sduenho = cam.getIdcliente(MenuEditarMotosDeCampoLista.id);
        smotivoVisita = cam.getMotivoVisita(MenuEditarMotosDeCampoLista.id);
      ...
      //Creanse paneles e daselle un color de fondo
        ...
        label = new JLabel("EDITAR MOTO DE CAMPO");
	...
	public void actionPerformed(ActionEvent e) {	
		...
		if (e.getActionCommand().equals("ATRÁS")) {
			MenuMotosDeCampo men = new MenuMotosDeCampo();
			dispose();
			men.setVisible(true);
		...
		if (e.getActionCommand().equals("ENVIAR")) {
	        ...
	        if (cam.editar(MenuEditarMotosDeCampoLista.id, inputmarca.getText(), inputmodelo.getText(), inputmatricula.getText(), combus, inputnPuertas.getText(), inputnRuedas.getText(), inputmaxVelocidad.getText(), inputcapMotor.getText(), inputcapAlmacenamiento.getText(), inputnPasajeros.getText(), a, g, airb, clim, des, idcli, inputmotivoVisita.getText())) {
				MenuListadoMotosDeCampo men = new MenuListadoMotosDeCampo();
				dispose();
				men.setVisible(true);
				...
				MenuErrorRegistroMotoDeCampo men = new MenuErrorRegistroMotoDeCampo();
				men.setVisible(true);
				dispose();
